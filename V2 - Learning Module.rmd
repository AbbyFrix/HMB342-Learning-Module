...
## 1.3.2 using conditions and logical statements 
...
```{r exercise 4, exercise = TRUE, exercise.lines = 6, exercise.setup="exercise4setup"}
cannabis_data <- read.csv("cannabis_data.csv")
# Apply the same filtering and selection done in the main module
...
```{r exercise 4, exercise = TRUE, exercise.lines = 6}
...
## 4.1 When is a dataset considered tidy?
...
## 4.2 Wrangling & Summarizing Data Part 1 - Categorical Data 
...
Whether categorical data is *ordinal* or *nominal* should be considered in which 
visualization type would be the most effective. Bar charts and pie charts would likely be more appropriate visualizations for nominal data, while visualizations that emphasize meaningful order or progression, such as ordered bar charts or line charts, would be more effective for displaying ordinal data. We will look deeper into data visualizations in a later section. 
...
## 4.3 Wrangling & Summarizing Data Part 2 - Numerical Data 
...
```{r exercise5setup, echo = FALSE}
pfas_data <- read.csv("pfas_data.csv")
```


```{r exercise 5, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise5setup"}
#mean_pfos_by_sample<- 
...
```{r, exercise 5 solution,messag=FALSE, echo=FALSE, results=FALSE}
# SOLUTION
mean_pfoa_by_sample <- pfas_data %>%
  filter(Analyte == "PFOA") %>% #filtering to only get PFOA
  group_by(Utility) %>% 
...
```{r exercise 8, exercise = TRUE, exercise.lines = 8, exercise.setup="exercise8setup"}
...
```{r exercise 9, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise9setup"}
...
```{r exercise 10, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise10setup"}
pfas_data_mean %>% 
...
```{r exercise 12, exercise = TRUE, exercise.lines = 8, exercise.setup="exercise12setup"}
cannabis_data %>% 
...
## 5.3 Plotting More than One Variable 

### 5.3.1 Using *facet_wrap* to show visualizations by group 
...
```{r exercise 13, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise 13setup"}
...
### 5.3.2 Using *boxplots* to represent multivariable data
...
```{r exercise14setup, echo = FALSE}
pfas_data <- read.csv("pfas_data.csv")
```


```{r exercise 14, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise14setup"}
pfas_data %>%
  filter(State == "NJ") %>%
  group_by(Analyte) %>%
...
```{r exercise 15, exercise = TRUE, exercise.lines = 5, exercise.setup="exercise15setup"}
...
Using cannabis_data, assume we want to look at the distribution of use fequency across different education levels. Write a code to produce this visualization:

```{r exercise16setup, echo = FALSE}
cannabis_data <- read.csv("cannabis_data.csv")
cannabis_data <- cannabis_data %>% 
  select(prov, education3, income_recode, community_size, use_freq_rec) %>%
  filter(if_all(everything(), ~ . != -7)) %>%
...
```{r exercise 16, exercise = TRUE, exercise.lines = 6, exercise.setup="exercise16setup"}
...
Using the PFAS births data, create a data visulization of the relationship between PFAS exposure and fertility rate. 


```{r exercise17setup, echo = FALSE}
pfas_data <- read.csv("pfas_data.csv")
birth_data <- read.csv("birth_data.csv")
birth_data <- birth_data %>% rename(State = State.Abbreviation)
pfas_data_mean <- pfas_data %>%
...
```{r exercise 17, exercise = TRUE, exercise.lines = 6, exercise.setup="exercise17setup"}
...
## 5.5 Base R Functions - Numerical Data
...
## 5.6 Visualizing Data Using Base R Functions - Categorical Data, One Variable
...
### 5.6.1 *frequency table*

A frequency table lists a set of values and how often each set of values appears. This frequency table looks at how many observations are for each classification of recreational cannabis use frequency: 
...
### 5.6.2 *proportion table*

A proportion table, also known as a relative frequency table, is essentially a frequency table that shows the propotion of a categories frequency relative to the total number of observations. For example: 

```{r}
prop.table(table(cannabis_data$recreational_use_frequency))
```

### 5.6.3 *bar plots* (using base R)
...
### 5.6.4 *pie chart* (there is no ggplot pie chart!)

Pie charts are categorical data visualizations where each "slice" represents a propotion of the whole. An example: 

```{r}
pie(table(cannabis_data$recreational_use_frequency))
```

Importantly, pie charts have a lot of shortcomings and are often not the most effective visualizations. Having too many categories/"slices", having multiple similar values between categories, and ineffective visual display of exact proportions are some problems that can occur. In most instances, it is simply more effective to use bar charts given their versatility and increased informational capacity. Pie charts are only really an effective display when a part-to-whole comparison is meaningful, there are few categories, and the slices carve out identifiable propotions in the chart (such as multiples of 1/4, or 1/3).


### 5.6.5 how to customize base R plots
...
## 5.7 Visualizing Data Using Base R Functions - Categorical Data, Two Variables

We can create a two-way table using `dplyr` code, but base R code is shorter. Consider variables `annual_income` and `recreational_use_frequency``. We can produce two-way tables of counts and proportions and obtain joint distributions and conditional distributions.

### 5.7.1 *two-way table of counts*
...
### 5.7.2 *two-way table of proportions* or *joint distribution*
...
### 5.7.3 *two-way table of proportions* or *conditional distribution given row variable*

```{r}
prop.table(table(cannabis_data$annual_income,cannabis_data$recreational_use_frequency),margin=1)
```

### 5.7.3 *two-way table of proportions* or *conditional distribution given column variable*

```{r}
prop.table(table(cannabis_data$annual_income,cannabis_data$recreational_use_frequency),margin=2)

```
### 5.7.4 *barplots* and *mosaic plots*

If we are interested in comparing annual income classifications versus cannabis usage frequency, we can use the second conditional distribution table above, or visualize this in a barplot or a mosaic plot:
```{r}
...
## 5.8 Relative Risks and Odds Ratios
...
```{r exercise 18, exercise = TRUE, exercise.lines = 10, exercise.setup="exercise18setup"}
...
Compare the above version of the map with this one, run the code below: 
```{r}
ggplot(states_map, aes(x = long, y = lat, group = group, fill = mean_ppt)) + 
...
Going back to the beginning of this section, think about which map displays the data more meaningfully and is more visually appealing, and why this is the case. 

Let's move on to the next component of an effective visualization!

## 6.2 *Labeling Your Data and Text* 
...
### 6.2.3 *Titles and Fonts* 

- Use readable font sizes 
- Keep titles concise but informative
- Use consistent font throughout your visualization

### 6.2.4 *Putting it all Together: Layout, Structure and Purpose*
...
This section covers a lot of ground, so let's see what some examples would look like in practice: 

```{r exercise19setup, echo=FALSE}
total_pfas_data <-pfas_data %>%
  filter(Analyte == "Total PFAS") 
...
If you saw this visualization given no context, you would not be able to make any sense of it for a variety of reasons. So let's transform it step by step: 

```{r}
total_pfas_data %>% 
  ggplot(aes(x = Value..PPT.)) + 
  geom_histogram(binwidth = 5, fill = "steelblue", color = "white", alpha = 0.8) + #binwidth` or `bins` arguments can be adjusted to change the size of the intervals the values fall under
  scale_x_continuous(
    limits = c(0, 100),
    breaks = seq(0, 100, by = 10), 
    labels = seq(0, 100, by = 10) # creating appropriate axis units
  ) +
  labs(
    title = "Distribution of Total PFAS Measurements in Water Samples", #clear and descriptive title
    x = "Total PFAS Concentration (ppt)", #informative axes titles 
    y = "Number of Samples" #changing 'count' to a more meaningful variable name 
  ) +
  theme_minimal() +
  theme(
  ...
  ) #removing unnecessary clutter and minimizing gridlines 
...
Feel free to change any of the elements, and rerun the code to see how the visualization changes!
